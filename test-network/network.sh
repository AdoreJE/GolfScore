...
function createOrgs() {
  ...
  echo
  echo "Generate CCP files for Sales1 and Customer"
...
function updateCC() {
  ...
  if [ $? -ne 0 ]; then
    echo "ERROR !!! Updating chaincode failed"
    exit 1
  fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/sales1/msp organizations/fabric-ca/sales1/tls-cert.pem organizations/fabric-ca/sales1/ca-cert.pem organizations/fabric-ca/sales1/IssuerPublicKey organizations/fabric-ca/sales1/IssuerRevocationPublicKey organizations/fabric-ca/sales1/fabric-ca-server.db'
...
if [ "$MODE" == "up" ]; then
  ...
  echo "updating chaincode on channel '${CHANNEL_NAME}'"
  echo
else
  printHelp
  exit 1
